#ifndef TRIGGERPI_BUILTIN_TIME_TRIGGER_H
#define TRIGGERPI_BUILTIN_TIME_TRIGGER_H
...
/*
  Built in time/interval based trigger
...
class builtin_time_trigger :public expansion_board {
  public:
    typedef TimeBaseT time_base;

    // trigger constantly between \c start and \c stop
    template<typename Clock1, typename Clock2>
    builtin_time_trigger(typename std::chrono::time_point<Clock1> start,
      typename std::chrono::time_point<Clock2> stop);

    template<typename Clock>
    builtin_time_trigger(time_base start,
      typename std::chrono::time_point<Clock> stop);

    template<typename Clock>
    builtin_time_trigger(typename std::chrono::time_point<Clock> start,
      time_base stop);
    ...
    template<typename Clock>
    builtin_time_trigger(typename std::chrono::time_point<Clock> start, bool);

    builtin_time_trigger(time_base start, bool);


    // trigger constantly starting immediately and stopping at \c stop
    template<typename Clock>
    builtin_time_trigger(bool, typename std::chrono::time_point<Clock> stop);

    builtin_time_trigger(bool, time_base stop);
    ...
    */
    template<typename Clock>
    builtin_time_trigger(time_base on_dur,
      time_base off_dur,
      typename std::chrono::time_point<Clock> stop);

    builtin_time_trigger(bool, time_base on_dur,
...
template<typename Clock1, typename Clock2>
builtin_time_trigger<TimeBaseT>::builtin_time_trigger(
  typename std::chrono::time_point<Clock1> start,
  typename std::chrono::time_point<Clock2> stop)
    :expansion_board(trigger_type_t::single_source)
{
  _run = [=](void) {
...
template<typename TimeBaseT>
builtin_time_trigger<TimeBaseT>::builtin_time_trigger(time_base start,
  time_base stop)
    :expansion_board(trigger_type_t::single_source)
{
  _run = [=](void) {
...
template<typename TimeBaseT>
template<typename Clock>
builtin_time_trigger<TimeBaseT>::builtin_time_trigger(bool,
  typename std::chrono::time_point<Clock> stop)
    :expansion_board(trigger_type_t::single_source)
{
  _run = [=](void) {
...
builtin_time_trigger<TimeBaseT>::builtin_time_trigger(
  ...
  time_base on_dur, time_base off_dur,
  typename std::chrono::time_point<Clock2> stop)
    :expansion_board(trigger_type_t::multi_source)
{
  _run = [=](void) {
    std::this_thread::sleep_until(start);
...
make_builtin_time_trigger(const std::basic_string<CharT> &raw_str)
{
  typedef std::basic_string<CharT> string_type;
  typedef std::chrono::nanoseconds time_base;
  typedef builtin_time_trigger<time_base> builtin_trigger_type;
